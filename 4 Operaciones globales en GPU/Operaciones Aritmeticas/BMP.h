...
struct BMP {
    ...
    void write_image_data(const char* fname, vector<uint8_t> nueva_data) {
        std::ofstream of{ fname, std::ios_base::binary };
        if (of) {
            if (bmp_info_header.bit_count == 32) {
                write_headers(of);
                of.write((const char*)nueva_data.data(), nueva_data.size());
            }
            else if (bmp_info_header.bit_count == 24) {
                if (bmp_info_header.width % 4 == 0) {
                    write_headers(of);
                    ...
                    for (int y = 0; y < bmp_info_header.height; ++y) {
                        of.write((const char*)(nueva_data.data() + row_stride * y), row_stride);
                        of.write((const char*)padding_row.data(), padding_row.size());
                    }
                }
            }
            else if(bmp_info_header.bit_count == 8) {
                uint32_t new_stride = 1;//make_stride_aligned(1);
                std::vector<uint8_t> padding_row(new_stride - row_stride);
